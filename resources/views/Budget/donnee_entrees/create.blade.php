...
@section('content')
    <div class="container">
        <h3 class="mb-4">➕ Nouvelle donnée budgétaire d’entrée</h3>

        <form method="POST" action="{{ route('donnee_entrees.store') }}">
            @csrf
            <div id="elements-container">
                <div class="row element-row border p-3 mb-3">
                    <div class="col-md-6 mb-3">
                        <label>Libellé</label>
                        <input type="text" name="donnee_ligne_budgetaire_entree[]" class="form-control" required>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label>Code</label>
                        <input type="text" name="code_donnee_budgetaire_entree[]" class="form-control" required>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label>Numéro compte</label>
                        <input type="text" name="numero_donnee_budgetaire_entree[]" class="form-control" required>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="id_ligne_budgetaire_entree[]" class="form-control select2" required>
                            <option value="">-- Sélectionner --</option>
                            @foreach($lignes as $l)
                                <option value="{{ $l->id }}">{{ $l->libelle_ligne_budgetaire_entree }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
...
@section('breadcrumb')
    <ol class="breadcrumb" style="background-color: transparent;padding: 4px 10px">
        ...
        <li><a href="{{ route('donnee_entrees.index')}}"><strong>Donnée </strong></a></li>
