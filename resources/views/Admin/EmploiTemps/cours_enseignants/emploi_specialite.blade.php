...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="no-print">
            <a href="{{ route('cours_enseignants.emploi_specialite_pdf', request()->query()) }}" class="btn btn-danger">
                PDF
            </a>
    ...
    <form method="GET" class="card p-3 mb-4 no-print">
        <div class="row">
            ...
            <div class="col-md-2 mt-2">
                ...
                <select name="type_personnel" class="form-control">
                    <option value="">Tous</option>
                    @foreach($typesPersonnel as $value => $label)
                        <option value="{{ $value }}" {{ request('type_personnel') == $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-2 mt-2">
                <label>Jour / Soir</label>
                <select name="periode_journee" class="form-control">
                    <option value="">Tous</option>
                    @foreach($periodesJournee as $value => $label)
                        <option value="{{ $value }}" {{ request('periode_journee') == $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-2 mt-2">
                <label>Format plage</label>
                <select name="format_plage" class="form-control">
                    <option value="">Tous</option>
                    @foreach($formatsPlage as $value => $label)
                        <option value="{{ $value }}" {{ request('format_plage') == $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
                ...
                <tr>
                    ...
                    @foreach($plages as $plage)
                        <th class="text-center">
                            {{ substr($plage->heure_debut, 0, 5) }} - {{ substr($plage->heure_fin, 0, 5) }}
                        </th>
                    @endforeach
                </tr>
            </thead>
    ...
    <div class="row mt-5 signature-zone">
