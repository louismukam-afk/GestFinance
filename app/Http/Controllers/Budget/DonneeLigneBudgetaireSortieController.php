...
class DonneeLigneBudgetaireSortieController extends Controller
{
    ...
    {
        return (float) donnee_ligne_budgetaire_sortie::where('id_donnee_budgetaire_sortie', $donneeId)
            ->when($excludeId, fn($query) => $query->where('id', '<>', $excludeId))
            ->sum('montant');
    }

    private function validateMontantDisponible(donnee_budgetaire_sortie $donnee, float $montantAjoute, ?int $excludeId = null): void
    {
    ...
    public function update(Request $request, $id)
    {
        ...
        $donnee = donnee_budgetaire_sortie::findOrFail($ligne->id_donnee_budgetaire_sortie);
